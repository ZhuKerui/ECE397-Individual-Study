...
class cluster:
    ...
    def get_related(self, central_word, threshold, use_topic=True, similariy=True):
        if use_topic:
            return self._get_similar(simple_normalize(self._t_vecs[self._t_w2i[central_word]]), ugly_normalize(self._t_vecs), self._t_vocab, threshold, similariy)
        else:
            return self._get_similar(simple_normalize(self._f_vecs[self._f_w2i[central_word]]), ugly_normalize(self._f_vecs), self._f_vocab, threshold, similariy)
