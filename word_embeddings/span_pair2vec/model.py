...
from span_pair2vec.representation import SpanRepresentation
...
class Pair2Vec(Module):
    def __init__(self, config, arg_vocab, rel_vocab):
        ...
        self.represent_argument = SpanRepresentation(config, config.d_args, arg_vocab)
    ...
    def forward(self, batch):
        ...
        sampled_subjects = sampled_subjects.view(-1, subjects.size(1), 1).squeeze(-1)
        sampled_objects = sampled_objects.view(-1, objects.size(1), 1).squeeze(-1)
        
        subjects, objects, sampled_subjects, sampled_objects = self.to_tensors((subjects, objects, sampled_subjects, sampled_objects))
        # embedded_subjects = self.represent_left_argument(subjects)
        # embedded_objects = self.represent_right_argument(objects)
        embedded_subjects = self.represent_argument(subjects)
        embedded_objects = self.represent_argument(objects)
        ...
        if sampled_subjects is not None and sampled_objects is not None:
            ...
            sampled_subjects = self.represent_argument(sampled_subjects)
            sampled_objects = self.represent_argument(sampled_objects)
